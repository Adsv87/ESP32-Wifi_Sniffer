...
{	
	...
	while(true){
		if (esp_log_timestamp() - startTime >= 10000 && running == false) {
			running = true;
...
static void mqtt_app_start(void)
{
	...
	msg_id = esp_mqtt_client_publish(client, CONFIG_TOPIC, string, strlen(string), 0, 0);
	ESP_LOGI(TAG, "[WI-FI] Sent publish successful on topic=%s, msg_id=%d", CONFIG_TOPIC, msg_id);
}
